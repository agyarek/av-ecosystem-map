...
- Standfirst: display face (the body's own family, not serif), `var(--ink-2)`, `var(--fs-lg)`,
  `var(--lh-body)` or looser, max-width `var(--measure)`, 180–340 characters,
  terminal-punctuated, first person allowed.
...
width. H2s are numbered and set with `var(--head-section)`. The number is generated by a CSS
counter (`decimal-leading-zero`: 01, 02, … — it cannot drift and is never typed into markup) and
renders mono, weight 600, `var(--muted)`, baseline-aligned with a `var(--sp-md)` gap before the
display-face heading text. The number is chrome: it does not count toward the
sentence-form/terminal-period rule. A labelled, uncounted variant (`.sec-mark`, e.g. Sources)
carries its mark text in place of a number. Callouts (`.note`, yellow
...
- Bio footer: quiet-overline label + first-person bio (no portrait or monogram — the
  person is carried by the LinkedIn-linked name), EMAIL ME button, fine print at
  `var(--measure-wide)`. The correction mailto is the shared `AV.CORRECTION` constant
  surfaced on the instruments (map selection card, ledger detail), not a footer element
  (full anatomy and addresses: COMPONENT_SPECIFICATIONS §4). Ends on a person — this is
  the site's CTA policy; no nav-level CTA exists.
...
band stays on the `var(--col)` container. Bands host text, labels, and hairline-tier
content only — **never sheets, cards, or forms**: in dark, `var(--med-bg)` sits tonally
between the ground and the sheet (`var(--paper-2)` is darker than the tint), so a sheet
placed on a band reads as recessed rather than raised. When a band "frames" an
instrument section, the band carries the framing prose and headings; the instrument's
own sheets sit on the ground before or after it. Partnerships and economics are mandated
...
3. **On-this-page TOC (secnav)** on article-class pages only. At ≥ grid width it
   ...
   column (S2-21) — reserved space, never floating over the text it indexes (rail
   geometry: 24ch wide, `var(--sp-2xl)` gap off the article column, sticky below the
   header; it auto-opens only at ≥ wide — COMPONENT_SPECIFICATIONS §22). Below
...
- Tables inside the article take `max-width: var(--measure-wide)` unless the data
  genuinely needs more; numeric columns take `ch`-based widths so the row-header
  column never strands right-aligned figures across a dead field
  (COMPONENT_SPECIFICATIONS §11).
...
## 6. Appendix A — the IA constant (mirror)

Source of truth is core.js CHAPTERS; this table is a mirror for doc-only consumers — verify
against core.js when editing.

| # | Chapter | Pages (key · path · label) |
|---|---------|----------------------------|
| 01 | Overview | `overview` · `overview/` · The industry in plain terms<br>`beyond-roads` · `beyond-roads/` · Autonomy off the road<br>`owning-one` · `owning-one/` · Buying one yourself |
| 02 | Map | `map` · `map/` · The ecosystem map |
| 03 | Directory | `companies` · `companies/` · Every organisation, every field<br>`operators` · `companies/passenger-autonomy/` · Passenger autonomy<br>`partnerships` · `partnerships/` · Who works with whom |
| 04 | Economics | `economics` · `economics/` · Funding: who raised what<br>`economics-unit` · `economics/#unit-economics` · Unit economics<br>`economics-compare` · `economics/#comparing-the-operators` · Comparing the operators |
| 05 | Regulatory | `regulation` · `regulation/` · Who decides, and every incident<br>`safety` · `safety/` · The safety evidence |
| 06 | Media | `media` · `media/` · Who to read, listen to, and meet |

Chapter numbers derive from array position (`pad2(i + 1)`), never from stored strings.
Single-page chapters (Map, Media) take their header dropdown menus from the separate
`SOLO_MENUS` constant in core.js, not from CHAPTERS — the chapters block ignores SOLO_MENUS.
